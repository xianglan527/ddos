...
#define __vop_info(node, type)                                       \
    ({                                                               \
        Inode *__node = (node);                                      \
        assert(__node != nullptr && check_inode_type(__node, type)); \
        &(__node->in_info.__##type##_info);                          \
    })

#define vop_info(node, type) __vop_info(node, type)

#define info2node(info, type) to_struct((info), Inode, in_info.__##type##_info)
...
struct inode_ops {
    ...
    int (*vop_read)(Inode *node, Iobuf *iob);
    int (*vop_write)(Inode *node, Iobuf *iob);
    ...
    int (*vop_readlink)(Inode *node, Iobuf *iob);
    int (*vop_symlink)(Inode *node, const char *name, const char *path);
    int (*vop_namefile)(Inode *node, Iobuf *iob);
    int (*vop_getdirentry)(Inode *node, Iobuf *iob);
...
#define __vop_op(node, sym)                                                                             \
    ({                                                                                                  \
        Inode *__node = (node);                                                                         \
        assert(__node != nullptr && __node->in_ops != nullptr && __node->in_ops->vop_##sym != nullptr); \
        inode_check(__node);                                                                            \
