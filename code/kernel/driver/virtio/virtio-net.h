#ifndef __VIRTIO_VIRTIO_NET_H_
#define __VIRTIO_VIRTIO_NET_H_
...
#define VIRTIO_NET_F_HASH_REPORT \
    (57)  // Device can report per-packet hash value and a type of calculated hash.
...
#define VIRTIO_NET_F_RSC_EXT \
    (61)  // Device can process duplicated ACKs and report number of coalesced segments and duplicated ACKs.
#define VIRTIO_NET_F_STANDBY \
    (62)  // Device may act as a standby for a primary device with the same MAC address.
...
#define VIRTIO_NET_HDR_GSO_ECN 0x80
    ...
    // uint16_t num_buffers; (Only if VIRTIO_NET_F_MRG_RXBUFT negotiated)
...
#define VIRTIO_NET_PKT_LEN 1514
    uint8_t pkt[VIRTIO_NET_PKT_LEN];
} __attribute__((packed));
